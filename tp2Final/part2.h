...
bool getWordFromFile(char* word, char* sentence, int* size, int* skip, int* inSent, ifstream& fin);
...
void findWordInFile(char* word) {
	...
	char c;
	while (1) {
		if (!getWordFromFile(buffer, sentence, &size, &skip, &inSentence, fin)) {
			if (compare(buffer, word) && skip != 1) flag = 1;
			if (flag) {
				numOfSentenses++;
				sentence[size] = '\0';
			...
			if (compare(buffer, word) && skip != 1) flag = TRUE;
		}
...
bool getWordFromFile(char* word, char* sentence, int* size, int* skip, int* inSent, ifstream& fin) {
	...
	if (endOfSentence(c)) {
		*inSent = 0;
		word[wc] = '\0';
		sentence[(*size)++] = c;
		//while (!(((c = fin.peek()) > 'A' && c < 'Z') || (c >= 'А' && c <= 'Я')) && !fin.eof()) fin.get();
		return FALSE;
	}
	sentence[(*size)++] = c;
	word[wc] = '\0';
